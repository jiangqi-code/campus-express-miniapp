...
const getTaskId = (item: OrderItem): string | number => {
  return item.task_id || item.task?.id || item.id || ''
}

const getOrderId = (item: OrderItem): string | number => {
...
const contactRunner = (item: OrderItem) => {
  ...
  uni.navigateTo({
    url: `/pages/task/detail?id=${id}&chat=true`
  })
}
...
const handleCancel = (item: OrderItem) => {
  ...
  uni.showModal({
    ...
    success: async (res) => {
      ...
      const id = isOrder ? getOrderId(item) : getTaskId(item)
      if (!id) {
        uni.showToast({ title: isOrder ? '订单ID不存在' : '任务ID不存在', icon: 'none' })
        return
      }
      try {
        if (isOrder) {
...
const submitRefund = async () => {
  ...
  const id = refundModal.order ? getOrderId(refundModal.order) : ''
  if (!id) return
  try {
