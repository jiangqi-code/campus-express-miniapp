...
async function handleUnauthorized() {
  ...
  if (currentRoute === 'pages/auth/index') return
  ...
  uni.reLaunch({
    url: `/pages/auth/index?redirect=${encodeURIComponent(`/${currentRoute}`)}`,
...
function normalizeNetworkFailure(err: any) {
  const message = String(err?.errMsg || err?.message || '').toLowerCase()
  if (message.includes('timeout')) return new Error('请求超时，请检查网络后重试')
  if (message.includes('fail') || message.includes('network') || message.includes('offline')) {
    return new Error('网络连接不可用，请检查网络设置')
  }
    ...
    uni.request({
      ...
      success: (res) => {
        ...
        if (statusCode < 200 || statusCode >= 300) {
          if (statusCode === 401) void handleUnauthorized()
        ...
        xhr.onload = () => {
          try {
            ...
            if (status < 200 || status >= 300) {
              if (status === 401) void handleUnauthorized()
