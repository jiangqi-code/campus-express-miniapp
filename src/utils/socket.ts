...
class MiniSocket {
  ...
  setBackground(value: boolean) {
    ...
    if (value) this.pauseHeartbeat()
    else this.reconnectNow()
  }

  private scheduleReconnect() {
    if (this.reconnectTimer || this.manualClose || !this.token) return
    const base = Math.min(30000, 1000 * 2 ** Math.min(this.reconnectAttempts, 5))
    const delay = this.background ? Math.max(base, 10000) : base + Math.floor(Math.random() * 500)
  ...
  sendEvent(event: string, data: any) {
    ...
    this.sendRaw(`42${JSON.stringify([event, data])}`)
  }
